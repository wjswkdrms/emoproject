      ...
      <c:forEach var="zzim" items="${list}">
        <ul class="list">
          ...
          <li class="list-date">${zzim.order_date}</li>
            ...
            <%-- <c:if test="${zzim.order_status==0}"><li class="list-write">배송 완료 후 후기 작성 가능</li></c:if>--%>
            <c:if test="${zzim.order_status<=1}"><li class="list-write">배송 완료 후 후기 작성 가능</li></c:if>
            <c:if test="${zzim.order_status==2 && zzim.review_num == 0}"><li class="list-write"><a class="write" onclick="location.href='productAfterWriteForm.do?product_num=${zzim.product_num}&order_num=${zzim.order_num}'">후기 작성</a></li></c:if>
         	<c:if test="${zzim.order_status==2 && zzim.review_num != 0}"><li class="list-write">후기 작성 완료</li></c:if>
        </ul>
      </c:forEach>
